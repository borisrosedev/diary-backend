...
import callAtlas from "../database/client.mjs";
import tryCatch from "../helpers/tryCatch.mjs";
...
const userController = {
    ...
    try {
      ...
      if (!isPasswordValid) {
        ...
        try {
          ...
          return res
            .status(500)
            .json({ message: "Error while inserting data in logs diary" });
        }
        client.close();
  ...
  async createOne(req, res) {
    ...
    tryCatch(
      ...
      res,
      "⛔️ Error while inserting data into the db",
    );
  },
  ...
  async deleteOneById(req, res, next) {
    ...
    try {
      ...
      return res
        .status(200)
        .json({ message: `User with id ${req.body.id} deleted`, result });
    } catch (err) {
      client.close();
      return res
        .status(500)
        .json({ message: `Error while retrieving date from the db` });
    }
  },
